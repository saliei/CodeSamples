...
{
    return sprocess(begin, end, [](auto a, auto b){return *std::min_element(a, b);});
}

template <typename Iterator>
auto smax(Iterator begin, Iterator end)
{
    return sprocess(begin, end, [](auto a, auto b){return *std::max_element(a, b);});
}
...
auto pprocess(Iterator begin, Iterator end, F&& f)
{
    ...
    if(size <= 10000) return std::forward<F>(f)(begin, end);
    else
    {
...
{
    return pprocess(begin, end, [](auto a, auto b){return *std::min_element(a, b);});
}

template <typename Iterator>
auto pmax(Iterator begin, Iterator end)
{
    return pprocess(begin, end, [](auto a, auto b){return *std::max_element(a, b);});
}
