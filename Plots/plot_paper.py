...
plot_types={
        "knl": {"data": ["mlups-KNL-1MPI-256THREADS.dat"],
        ...
        "data_structures": {"data": ["mlups-256-AVGxCall-MARCONI100-1MPI-v100.dat"],
        ...
        "strong_poiseuille_512": {"data": ["mlups-AVGxCall-poiseuille-strong-512.dat"],
        ...
        "strong_poiseuille_256": {"data": ["mlups-AVGxCall-poiseuille-strong-256.dat"],
        ...
        "speedup_cpu_gpu": {"data": ["mlups-poiseuille-move-collide-cpu-gpu-256.dat"],
        ...
        "multi_component_opt_non_opt": {"data": ["mlups-AVGxCall-multi-component-256-non-opt.dat", "mlups-AVGxCall-multi-component-256-opt.dat"],
                                        ...
                                        "l2loc": "upper right",
                                        "cmap": "BrBG",
                                        "cmap_range": (0.55, 0.99),
                                        "width": 1.3,
                                        },
        "strong_mulit_component_256": {"data": ["mlups-AVGxCall-multi-component-strong-256.dat"],
        ...
        "strong_mulit_component_512": {"data": ["mlups-AVGxCall-multi-component-strong-512.dat"],
        ...
        "weak_multi_component": {"data": ["mlups-AVGxCall-multi-component-weak.dat"],
        ...
        "m100_mn4": {"data": ["mlups-AVGxCall-M100-MN4.dat"],
...
def plot_clustered_stacked(all_df, plot_type, plot_props, cmap="BrBG"):
    ...
    for i in range(0, n_df * n_col, n_col): # len(h) = n_col * n_df
        ...
        if "xlabelfontsize" in plot_props:
            if "xlabelrotation" in plot_props:
                ...
                rot = 0
            axe.set_xticklabels(all_df[0].index.values, rotation = rot, fontsize=plot_props["xlabelfontsize"])
        else:
            if "xlabelrotation" in plot_props:
                rot = plot_props["xlabelrotation"]
            else:
                rot = 0
        axe.set_xticklabels(all_df[0].index.values, rotation = rot)
        axe.set_xlabel(plot_props["xlabel"], fontproperties=font)
        # axe.set_title(plot_props["title"], fontproperties=font)
        axe.set_ylabel("MLUPs", fontproperties=font)


    if "l1bbox" in plot_props:
        if "l1ncol" in plot_props:
            if plot == "multi_component_opt_non_opt":
                l1 = axe.legend(h[:n_col], l[:n_col], loc=plot_props["l1loc"], bbox_to_anchor=plot_props["l1bbox"] ,ncol=plot_props["l2ncol"], prop={'size': 8}, frameon=False)
            else:
    ...
    if "labels" in plot_props:
        ...
        if plot == "multi_component_opt_non_opt":
            l2 = plt.legend(n, plot_props["labels"], loc=plot_props["l2loc"], bbox_to_anchor=(0.81, 0.99) ,ncol=1, prop={'size': 8}, frameon=False)
        else:
...
if __name__ == "__main__":
    ...
    # plot = "multi_component_opt_non_opt"
    plot = "weak_multi_component"
    ...
    if plot == "multi_component_opt_non_opt":
        dfs = [df.transform(lambda x: np.power(256, 3)/(x * np.power(10, 6))) for df in dfs]

    if plot == "weak_multi_component":
        df = dfs[0]
        df["mlups"] = df.sum(axis=1)
        for idx in df.index:

            if idx == 2:
                # df[df.index == idx] = df.loc[[idx]].transform(lambda x: np.power(256, 3)/(x * np.power(10, 6)))
                df["mlups"][idx] = np.power(256, 3) / (df["mlups"][idx] * np.power(10, 6))
            elif idx == 16:
                df["mlups"][idx] = np.power(512, 3) / (8 * df["mlups"][idx] * np.power(10, 6))
                # df[df.index == idx] = df.loc[[idx]].transform(lambda x: np.power(512, 3)/(x * np.power(10, 6)))
            else:
                df["mlups"][idx] = np.power(1024, 3) / (64 *df["mlups"][idx] * np.power(10, 6))
                # df[df.index == idx] = df.loc[[idx]].transform(lambda x: np.power(1024, 3)/(x * np.power(10, 6)))
    # print(df.sum(axis=1))
    dfs = [df.loc[:, df.columns == "mlups"]]
