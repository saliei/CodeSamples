...
title = "Weak Scaling, Emulsion - 4xTesla-V100-SXM2-16GB"
def plot_clustered_stacked(dfall, labels=None, title=title,  H="x", **kwargs):
    ...
    for i in range(0, n_df * n_col, n_col): # len(h) = n_col * n_df
        for j, pa in enumerate(h[i:i+n_col]):
            for rect in pa.patches: # for each index
                ...
                rect.set_width(1 / (2.3*float(n_df + 1)))

    # axe.set_xticks(np.arange(0, 2 * n_ind, 2))
    # axe.set_xticks(np.arange(0, n_ind))
    axe.set_xticks((np.arange(0, 2 * n_ind, 2) - 1.25 / (2.3*float(n_df + 1))) / 2.)
    if( n_df > 1 ):
    ...
    # print(eff)
    zoom_axe = inset_axes(axe, "44%", "30%", "upper right", borderpad=2)
